  ...
  properties: {
    ...
    choices: {
      ...
      items: {
        type: "object",
        additionalProperties: false,
        required: ["t", "d", "effect", "form", "rep", "fit"],
        properties: {
          t: { type: "string" },
          d: { type: "string" },
          effect: {
            ...
            additionalProperties: false,
            properties: {
              PAC: { type: "number", minimum: -2, maximum: 3 },
              SHO: { type: "number", minimum: -2, maximum: 3 },
              PAS: { type: "number", minimum: -2, maximum: 3 },
              DRI: { type: "number", minimum: -2, maximum: 3 },
              DEF: { type: "number", minimum: -2, maximum: 3 },
              PHY: { type: "number", minimum: -2, maximum: 3 }
            }
          },
          form: { type: "number", minimum: -2, maximum: 3 },
          rep: { type: "number", minimum: -3, maximum: 4 },
          fit: { type: "number", minimum: -8, maximum: 5 }
        }
      }
    }
  }
...
Write in Simplified Chinese. Create one detailed, novel-like event for this player. The event must be believable for the stated age, height, weight, club, league, position, current attributes, identity, and recent history. Do not claim invented real-world transfer news as fact. You may mention real players only as plausible opponents, teammates, or public figures when the context supports it. Avoid melodrama and keep the consequences proportionate.
...
${context.customChoice}`;
    ...
    if (!response.ok) {
      ...
      return Response.json({ error: "DeepSeek request failed", detail }, { status: response.status });
    }
